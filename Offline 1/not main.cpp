...
struct Node
{
    Node* parent; //Print korar jonno parent rakhte hobe
    int** current_board; //Ekekta node hocche ekekta 2d array
    int k;
    int x, y; //* ta kothay ache oita
    int cost;  //Eita actually final cost ta mane f=g+h ; eita hocche f
    int move; //root theke ashte koyta move lagche oitar hisab
};
...
struct Comparator
{
    ...
    {
        return (node1->cost) > (node2->cost); //min priority queue te eivabe dite hoy
    }
};
...
{
    int num_arr[k*k], counter = 0, inversionCount = 0;
    // copy the matrix in an array
    for(int i = 0; i < k; i++)
    {
        for(int j = 0; j < k; j++)
        {
            num_arr[counter] = initial[i][j];
            counter=counter+1;
        }
    }
    //count inversion
...
int CalculateCost(int func, int k, int** initial, int** goal, int move_cost)
{
    if(func == 1)
    {
        ...
        return move_cost + heu_cost_hamming;
    }
    if(func == 2)
    {
        int heu_cost_manhattan = Manhattan_Distance(k,initial);

        return move_cost + heu_cost_manhattan;
    }
    ...
    while (!puzzleboardopen.empty())
    {
        ...
        if (!isExist(puzzleboardclosed, current, k)) //alreaedy ache kina
        {
            ...
            if (flag == 1)
            {
                ...
                else if(method == 2) cout << "Manhattan:\n";
...
int main()
{
    int k, x, y; //x and y hocche star er initial position. 0 base index
